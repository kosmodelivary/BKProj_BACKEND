...
public class NoticeListController extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//페이징을 위한 로직 시작]
		//전체 레코드 수
		int totalRecordCount = noticeService.getTotalRecordCount();
		//페이지 사이즈
		...
		map.put("start", start);
		map.put("end", end);
		List<NoticeDto> 	notice	= noticeService.selectList(map);
		
		// 페이징용 문자열 생성
		String	pagingString	=
				PagingUtil.pagingBootStrapStyle(totalRecordCount, pageSize, blockPage, nowPage, req.getContextPath() + "/ADMIN/NOTICE/List.do?");
